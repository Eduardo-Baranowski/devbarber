...
export default ({show, setShow, user, service}) => {
  ...
  useEffect(() => {
    if (user.available) {
      ...
      for (let i = 1; i <= daysInMonth; i++) {
        ...
        let availablility = user.available.filter(e => e.date === selDate);

        newListDays.push({
          status: availablility.length > 0 ? true : false,
  ...
  useEffect(() => {
    if (user.available && selectedDay > 0) {
      ...
      if (availablility.length > 0) {
        setListHours(availablility[0].hours);
      }
    }
  ...
  return (
    <Modal transparent={true} visible={show} animation Type="slide">
      <ModalArea>
        <ModalBody>
